...
const LazyLogin = async () => {
    const LoginPage = await lazyLoad('./component/login.js', 'LoginPage');
    return LoginPage;
};

const LazyAdmin = async () => {
    const AdminPanel = await lazyLoad('./component/admin.js', 'AdminPanel');
    return AdminPanel;
};

const LazyUser = async () => {
    const UserPanel = await lazyLoad('./component/user.js', 'UserPanel');
    return UserPanel;
};

const LazyResearchChair = async () => {
    const ResearchChairPanel = await lazyLoad('./component/researchChair.js', 'ResearchChairPanel');
    return ResearchChairPanel;
};

const LazyExtensionChair = async () => {
    const ExtensionUserPanel = await lazyLoad('./component/extensionUser.js', 'ExtensionUserPanel');
    return ExtensionUserPanel;
};

const LazyEvaluator = async () => {
    const Evaluator = await lazyLoad('./component/evaluator.js', 'Evaluator');
    return Evaluator;
};

const LazyRdeOffice = async () => {
    const RdeOffice = await lazyLoad('./component/rdeStaff/script/rde.js', 'RdeOffice');
    return RdeOffice;
};

const LazyScanner = async () => {
    const Scanner = await lazyLoad('./component/scanner.js', 'Scanner');
    return Scanner;
};
