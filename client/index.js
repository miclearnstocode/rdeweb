...
async function checkAndProtectSession() {
    try {
        ...
        // Check if we need to redirect (status true means redirect)
        ...
        // Also check for UNKNOWN username via getUserName
        ...
        // If username is UNKNOWN or needs redirect, redirect to login
...
window.addEventListener('DOMContentLoaded', async () => {
    (async function(url) {
        ...
        // Public account support route
        if (url.split('/')[1] === 'accountSupport'){
            Render = Retrieval
            document.getElementById('root').appendChild(Render())
            return;
        }
        ...
        // If session is invalid, checkAndProtectSession already redirected
        if (!isSessionValid) {
            return;
        }
        
        // Session is valid, render the appropriate component
