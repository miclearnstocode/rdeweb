...
export const RdeDashboard = () => {
    ...
    const barChart = (title, items, labelKey) => {
        ...
        const bars = items.map((item, idx) => {
            ...
            return $({
                tag: 'div', style: { display: 'flex', flexDirection: 'column', alignItems: 'center', flex: '1', minWidth: '32px', gap: '4px' }, child: [
    ...
    const donutChart = (title, items, labelKey) => {
        ...
        items.forEach((item, idx) => {
            ...
            // stroke-linecap: round adds total strokeW to the visible length.
            // We subtract it and the gap to ensure segments are distinct.
            const dash = Math.max(0.1, sliceLen - gap - strokeW);
            // Center the segment within its percentage arc
    ...
    return $({
        ...
        style: {
            width: '100%', height: '100%', overflow: 'auto', backgroundColor: '#1e1e1e',
