                    ...
                    $({
                        ...
                        elementHandler:(el)=>{
                            ...
                            req.Post([
                                ...
                                {
                                    name: 'eventId',
                                    value: Path(4) || '' // Get eventId from URL
                                },
                                {
                                    name: 'categoryId', 
                                    value: Path(6) || 0 // Get category/center ID from URL
                                }
                            ]);
                            req.Send().then(data => {
                                // data is the array from rank.php
                                if (data && data.length > 0) {
                                    // Try to determine if we're using category or center system
                                    const firstDoc = data[0]?.docs?.[0]?.file;
                                    if (firstDoc) {
                                        if (firstDoc.center) {
                                            // New system with center
                                            CategoryName = firstDoc.center;
                                        } else if (firstDoc.category) {
                                            // Old system with category
                                            CategoryName = firstDoc.category;
                                        } else {
                                            CategoryName = 'All Entries';
                                        }
                                    } else {
                                        CategoryName = 'All Entries';
                                    }
                                    el.innerText = `Ranking Result for ${CategoryName}`;
                                } else {
                                    el.innerText = `Ranking Result for All Entries`;
                                }
                            }).catch(err => {
