...
export const PatentUM = () => {
    ...
    const columns = [
        ...
        { field: 'submissionStatus', header: 'Submission Status', width: '150px' },
    ...
    // Fetch next case number for automation
    const fetchNextCaseNumber = async (type) => {
        try {
            const formData = new FormData();
            formData.append('action', 'get_next_case_number');
            formData.append('type', type);
            const response = await fetch('/patentresearch', { method: 'POST', body: formData });
            const result = await response.json();
            return result.success ? result.next_case : '';
        } catch (e) {
            console.error('Error fetching next case number:', e);
            return '';
        }
    };
        ...
        const renderDynamicFields = (type, data = null) => {
            ...
            const campusSelect = $({
                ...
                child: [
                    ...
                    $({
                        tag: 'option',
                        att: { value: '', disabled: true, selected: !(data?.campus || data?.campusUM) },
                        text: 'Select Campus'
                    }),
                    // Actual campus options
            ...
            if (type === 'patent' || type === 'utility_model' || type === 'industrial_design') {
                ...
                const caseNumberInput = $({
                    ...
                    att: {
                        type: 'text',
                        name: 'caseNumber' + sfx,
                        placeholder: 'CAPSU IPMO 2026-001',
                        value: data?.['caseNumber' + sfx] || data?.caseNumber || '',
                        required: true
                    },
                    style: inputBaseStyle
                });
                ...
                // Automate case number for new records
                if (!data) {
                    caseNumberInput.placeholder = 'Fetching next case number...';
                    fetchNextCaseNumber(type).then(nextCase => {
                        if (nextCase) caseNumberInput.value = nextCase;
                        else caseNumberInput.placeholder = 'CAPSU IPMO 2026-001';
                    });
                }
                    ...
                    child: [
                        ...
                        $({
                            ...
                            child: [
                                ...
                                $({
                                    tag: 'button',
                                    text: isEdit ? 'Update Record' : 'Submit Record',
                                    style: {
                                    ...
                                    event: {
                                        ...
                                        method: async (e) => {
                                            ...
                                            formData.append('action', isEdit ? 'update_capsu' : 'save_capsu');
    ...
    const createPatentRow = (item) => {
        const row = $({
            ...
            child: columns.map(col => {
                ...
                } else if (col.field === 'submissionStatus') {
                    const statusVal = (item.submissionStatus || 'under review').toLowerCase();
                    const statusColor = statusVal === 'registered' || statusVal === 'accepted' ? '#4caf50' :
                        statusVal === 'rejected' ? '#f44336' : '#ff9800';
                    content = $({
                        tag: 'span',
                        style: {
                            padding: '4px 10px',
