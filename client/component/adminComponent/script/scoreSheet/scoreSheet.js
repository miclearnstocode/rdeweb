...
export const AddScoreSheet = ({ id, eventID, name }) => {
    ...
    const fetchCategories = async () => {
        try {
            ...
            req.Post([{ name: 'requestCat', value: '1' }])
            req.Json()
    ...
    const fetchCriteria = async () => {
        try {
            ...
            req.Post([
                { name: 'criteriaList', value: '1' },
                { name: 'scr_id', value: eventID }
            ])
            req.Json()
            const data = await req.Send()
            criteriaData = data || []
    ...
    const updateTable = () => {
        ...
        if (selectedCategory !== 'all') {
            filteredData = filteredData.filter(item => item.catId?.toString() === selectedCategory)
        }
                                        ...
                                        $({
                                            ...
                                            event: {
                                                ...
                                                method: async () => {
                                                    ...
                                                    form.append('centerId', categoryId)
